...
@SpringBootTest(properties = {
    "HR_CLIENT_SECRET=hr-test-secret-not-for-production",
    "AUTH_PUBLIC_URL=http://localhost:8080",
    "AUTH_INTERNAL_URL=http://localhost:8080",
    "HR_REDIRECT_URI=http://localhost/login/oauth2/code/hr-client",
    "HR_POST_LOGOUT_REDIRECT_URI=http://localhost:8081/",
    "SESSION_COOKIE_SECURE=false"
})
@AutoConfigureMockMvc
class HrServerApplicationTest {
    ...
    void startsAuthorizationCodeFlowWithPkce(@Autowired MockMvc mvc) throws Exception {
        String location = mvc.perform(get("/oauth2/authorization/hr-client"))
            .andExpect(status().is3xxRedirection())
            .andReturn().getResponse().getRedirectedUrl();
        var parameters = UriComponentsBuilder.fromUriString(location).build()
            .getQueryParams();
        ...
        assertThat(parameters.getFirst("redirect_uri"))
            .isEqualTo("http://localhost/login/oauth2/code/hr-client");
    }
    ...
    void initiatesExactRpLogoutAndRequiresCsrf(@Autowired MockMvc mvc,
        ...
        var registration = registrations.findByRegistrationId("hr-client");
        mvc.perform(post("/api/v1/logout").with(oidcLogin()
                .clientRegistration(registration).idToken(token -> token.claim("sid", "sid-hr"))))
            .andExpect(status().isForbidden());
        String location = mvc.perform(post("/api/v1/logout").with(oidcLogin()
                .clientRegistration(registration).idToken(token -> token.claim("sid", "sid-hr"))).with(csrf()))
        ...
        assertThat(query.getFirst("post_logout_redirect_uri")).isEqualTo("http://localhost:8081/");
