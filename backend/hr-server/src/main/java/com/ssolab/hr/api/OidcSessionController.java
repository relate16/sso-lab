...
public class OidcSessionController {
    ...
    private static final List<String> EXPOSED_CLAIMS = List.of("sub", "userId",
        "preferred_username", "username", "name", "email", "roles", "groups",
    ...
    Map<String, Object> session(@AuthenticationPrincipal OidcUser user, HttpSession session) {
        ...
        response.put("service", "HR");
        EXPOSED_CLAIMS.forEach(name -> {
            Object value = user.getClaims().get(name);
            if (value != null) response.put(name, value);
        });
        return response;
    }
}
