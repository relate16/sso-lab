...
function App() {
  ...
  if (!session) return <main className="center"><section className="login-card"><p className="eyebrow">SSO LAB · ADMIN</p><h1>Identity control plane</h1><p>ADMIN 역할과 loa:1 인증 컨텍스트가 필요합니다.</p><a className="primary" href="/oauth2/authorization/admin-client">관리자 SSO 로그인</a></section></main>

  return <main className="app-shell">
    <header><div><p className="eyebrow">SSO LAB · ADMIN</p><h1>Identity control plane</h1></div><div className="identity"><strong>{session.name || session.username}</strong><span>ADMIN · {session.acr}</span>{csrf && <form method="post" action="/api/v1/logout"><input type="hidden" name={csrf.parameterName} value={csrf.token} /><button type="submit">로그아웃</button></form>}</div></header>
